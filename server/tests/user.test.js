...
describe("User test", () => {
  beforeAll(async () => {
    await dao.run('DELETE FROM Users');
    // await db.run('DELETE FROM sqlite_sequence');
    await dao.run("INSERT OR IGNORE INTO Users(email, password, role, name, lastname, phone_number, salt)\
  ...
  afterAll(async () => {
    await dao.run('DELETE FROM HikePoint');
    await dao.run('DELETE FROM Points');
    await dao.run('DELETE FROM Hikes');
    await dao.run('DELETE FROM sqlite_sequence');
  });
